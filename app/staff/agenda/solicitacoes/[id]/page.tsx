...
import { ArrowLeft, Calendar, Clock, User, DollarSign, Check, X, Edit } from "lucide-react"
...
export default function GerenciarSolicitacao() {
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      const { error: aptError } = await supabase.from("appointments").insert(appointmentData)
  ...
  const handleReject = async () => {
    setIsLoading(true)
    try {
      const { error } = await supabase.from("appointment_requests").update({ status: "rejected" }).eq("id", request.id)
  ...
  const formatRequestedDateTime = () => {
    if (!request.preferred_date) return { date: "Data não especificada", time: "" }
  ...
  const { date: formattedDate, time: formattedTime } = formatRequestedDateTime()
              ...
              <Card className="border-gold/20">
                ...
                <CardContent className="space-y-3">
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Tem certeza que deseja aprovar esta solicitação? Um agendamento será criado com a data e
                          horário solicitados.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                  ...
                  <Button
                    ...
                    variant="outline"
                    className="w-full border-gold/20 text-gold hover:bg-gold/10 bg-transparent"
                    disabled={isLoading}
                  >
                    <Edit className="mr-2 h-4 w-4" />
                    {showModifyForm ? "Ocultar Modificação" : "Modificar Data/Horário"}
                  </Button>

                  {showModifyForm && (
                    <AlertDialog>
                      <AlertDialogTrigger asChild>
                        <Button className="w-full bg-gold hover:bg-gold/90 text-black" disabled={isLoading}>
                          <Check className="mr-2 h-4 w-4" />
                          Aprovar com Modificação
                        </Button>
                      </AlertDialogTrigger>
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          ...
                          <AlertDialogDescription>
                            Tem certeza que deseja aprovar esta solicitação com a nova data e horário? O cliente será
                            notificado sobre a mudança.
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
                          <AlertDialogCancel>Cancelar</AlertDialogCancel>
                          <AlertDialogAction onClick={handleApprove} className="bg-gold hover:bg-gold/90 text-black">
                            Confirmar
                          </AlertDialogAction>
                        </AlertDialogFooter>
                      </AlertDialogContent>
                  ...
                  <AlertDialog>
                    ...
                    <AlertDialogContent>
                      <AlertDialogHeader>
                        ...
                        <AlertDialogDescription>
                          Tem certeza que deseja rejeitar esta solicitação? O cliente será notificado sobre a rejeição.
                        </AlertDialogDescription>
                      </AlertDialogHeader>
                      <AlertDialogFooter>
                        <AlertDialogCancel>Cancelar</AlertDialogCancel>
